    ...
    <div className="p-8">
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2].map(i => (
            <div key={i} className="p-6 bg-surface border border-border rounded-sm animate-pulse h-32" />
          ))}
        </div>
      ) : projects.length === 0 ? (
            ...
            <div key={project.id} className="p-6 bg-surface border border-border rounded-none flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-xs px-2 py-1 bg-muted rounded-sm font-medium uppercase tracking-wider shrink-0 ml-2">{project.status}</span>
                </div>
                {project.description && (
